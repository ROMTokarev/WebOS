...
*/
	...
	function setCookie (name, value)
	{
      expires = 3000;
      var d = new Date();
      d.setTime(d.getTime() + expires*360000);
...
todo.prototype.drag=function(e, BOXID){

	var JTopCookie = parseInt(getCookie(BOXID+'[y]'));
	var JLeftCookie = parseInt(getCookie(BOXID+'[x]'));

	$('#'+BOXID).css("top", JTopCookie);
	$('#'+BOXID).css("left", JLeftCookie);
	...
	e._drag={'_':e,'x':null,'y':null,'draged':false,'onClick':e.onclick,
		'startDrag':function(event){
			...
			this.draged=false;
			if(this._.style.zIndex<todo.dragZIndex && window.LinkDrag==false)this._.style.zIndex=++todo.dragZIndex;
...
sleep(50).then(() => {
   ...
   todo.stopPropagation(event);
		...
		'stopDrag':function(event){
			...
			setCookie(BOXID+"[x]", event.clientX-this.x);
			setCookie(BOXID+"[y]", event.clientY-this.y);
...
window.StarDrag = true;
window.LinkDrag = false;
e.onmousedown=function(e){
	if (window.StarDrag) {
		this._drag.startDrag(e);}
	else {
		if(this.style.zIndex<todo.dragZIndex && window.LinkDrag==false) this.style.zIndex=++todo.dragZIndex;
  }
};
